...
N,lam=32,6.0
h=1.0/(N-1)
# DST-I fast Poisson solve on the interior: the exact inverse of the 5-point Dirichlet Laplacian
n=N-2
kk=np.arange(1,n+1)
eig=(2-2*np.cos(np.pi*kk/(N-1)))/(h*h)
DEN=eig[:,None]+eig[None,:]
def fastpoisson(v):
    w=np.zeros_like(v)
    rhs=v[1:-1,1:-1]
    w[1:-1,1:-1]=idstn(dstn(rhs,type=1)/DEN,type=1)
    return w
uref,kref=solve(N,lam,20,1e-4,50)
uf,kf=solve(N,lam,20,1e-4,50,fastpoisson)
print(f"jfnk_bratu N={N}: Newton runs to tolerance, so the inner preconditioner is FREE to change")
print(f"  reference (unpreconditioned matrix-free GMRES): newton={len(kref)} arnoldi={kref} total={sum(kref)}")
print(f"  DST fast-Poisson preconditioned inner GMRES  : newton={len(kf)} arnoldi={kf} total={sum(kf)}")
print(f"  matrix-free Jacobian applications saved: {sum(kref)} -> {sum(kf)}  ({sum(kref)/sum(kf):.1f}x fewer)")
print(f"  u differs from the reference by {viol(uf,uref):.3e} x the fp64 budget "
      f"({'PASSES' if viol(uf,uref)<=1.0 else 'FAILS'})")
